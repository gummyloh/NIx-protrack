-- Fixes "column reference \"project_id\" is ambiguous" on every attempt to
-- create a new project. The function's own RETURNS TABLE(project_id text, ...)
-- declares project_id as an output variable in scope for the whole function
-- body, which collided with two unqualified references to the tasks table's
-- own project_id column in WHERE clauses. Qualifying both with the table
-- alias resolves it.
CREATE OR REPLACE FUNCTION nixma.create_project_from_template(
  ...
  p_project_code text,
  p_kickoff_date date,
  p_source_project_id text DEFAULT 'liquick-go-pack-n-seal'::text
)
